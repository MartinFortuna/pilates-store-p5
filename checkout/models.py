...
class Order(models.Model):
    ...
    def update_total(self):
        ...
        """
        self.total_items = (
            self.orderitems.aggregate(Sum('quantity'))['quantity__sum'] or 0
        )
        self.grand_total = (
            self.orderitems.aggregate(Sum('subtotal'))['subtotal__sum'] or 0
        )
        if self.grand_total < settings.FREE_DELIVERY_THRESHOLD:
            self.delivery_cost = (
                self.grand_total * settings.STANDARD_DELIVERY_PERCENTAGE / 100
            )
        else:
