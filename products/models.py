...
class tbCategoryProduct(models.Model):
...
class tbSizeProduct(models.Model):
...
class tbProduct(models.Model):
    ...
    category_id = models.ForeignKey(tbCategoryProduct, on_delete=models.CASCADE, null=True)

    def __str__(self):
        return f"Product: {self.name} || Label: {self.label} || Price: {self.price} || {self.category_id}"


class tbInventoryProduct(models.Model):
    id_product = models.ForeignKey(tbProduct, on_delete=models.CASCADE)
    id_size = models.ForeignKey(tbSizeProduct, on_delete=models.CASCADE, null=True)
...
class tbRateProduct(models.Model):
